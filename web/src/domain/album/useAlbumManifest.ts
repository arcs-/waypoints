...
import { readAlbumFile, writeAlbumFile } from '@/domain/proton/files';
import type { AlbumRef } from './useAlbums';
import type { Photo, StoredAlbum } from './types';
...
export function useAlbumManifest() {
  ...
  async function ensure(album: AlbumRef, { rebuild = false } = {}) {
    ...
    try {
      ...
      const built = await buildManifest(p.photos, album, i18n.global.locale.value, (d, t) => { progress.value = d; total.value = t; });
