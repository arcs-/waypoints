...
import { CLIENT_UID_KEY } from '@/shared/lib/storageKeys';
...
// the last segment names THIS app (Waypoints → waypoints, no spaces/dashes). The version is
// injected from package.json at build time (vite define) — tauri.conf.json reads the same file.
const APP_VERSION = `external-drive-waypoints@${__APP_VERSION__}`;
...
function clientUid(): string {
  let id = localStorage.getItem(CLIENT_UID_KEY);
  if (!id) { id = crypto.randomUUID(); localStorage.setItem(CLIENT_UID_KEY, id); }
  return id;
}
